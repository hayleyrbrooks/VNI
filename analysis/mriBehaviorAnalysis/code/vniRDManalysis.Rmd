...
# QUESTION FOR PSH: I
# 1) INCLUDE SAFE IN RECEIVED-NOT? AND DOES THE NOT RECEIVED IN THIS CASE = 0 OR EV OF THE GAMBLE?
# 2) PLOTTING THIS
# use the received-not recieved regressor and past outcome type will be 1= gamble, 0 = safe
model2_potc_receivedDiff_type_intxn = glmer(choice ~ 0 + poc1type*pocRcvdMinusNotSC + (1|subjectIndex), data=mriBehClean, family="binomial", offset=pred);
summary(model2_potc_receivedDiff_type_intxn); # negative interaction between outcome type and amount; AIC = 7589.2

```


There is an interaction between outcome amount (received minus not recieved) and outcome type. Let's plot it.

```{r}
m2coefReceivedNot = fixef(model2_potc_receivedDiff_type_intxn); # store all beta estimates

typeCoef = m2coefReceivedNot[1]; # past outcome type coefficient
amtCoef = m2coefReceivedNot[2]; # past outcome amount coefficient
intxnCoef = m2coefReceivedNot[3]; # past outcome type and amount interaction coefficient


poc = seq(from = -1, to = 1, length.out=20);
# poc type is code 0 = safe, 1 = risky win, 2 = risky loss 

#safePgam = 1/(1+exp(-1*( (typeCoef*0) + (amtCoef*poc[6:10]) + (intxnCoef*0*poc[6:10]) ))); 

gainPgam = 1/(1+exp(-1*( (typeCoef*1) + (amtCoef*poc[(length(poc)/2 + 1):(length(poc))]) + (intxnCoef*1*poc[(length(poc)/2 + 1):(length(poc))]) ))); 
lossPgam = 1/(1+exp(-1*( (typeCoef*-1) + (amtCoef*poc[1:(length(poc)/2)]) + (intxnCoef*-1*poc[1:(length(poc)/2)]) ))); 

plot(poc,c(lossPgam, gainPgam), type="l", col="black", lwd=3, ylim=c(0,1), main="Risk-taking changes as a function of past outcome type and amount", ylab="p(gamble)", xlab="past outcome", axes=F)
#legend("topleft", legend=c("gain", "missed gain", "safe"), text.col = c("green", "red", "blue"), bty="n")
abline(a=.5,b=0, col="grey", lty="dashed")
abline(v=c(0,0), col="grey", lty="dashed")
axis(2)
axis(1, labels=round(poc, digits=2), at=poc)
#lines(gainPgam, col="blue", lwd=3)
#lines(lossPgam, pch=16, col="red", lwd= 3)
