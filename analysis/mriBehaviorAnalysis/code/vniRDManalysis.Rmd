...
# LEFT OFF HERE 6/15- HRB CONFUSED ABOUT BEST WAY TO PLOT THIS
...
gainOutcomes = c(.09, .5, 1);
lossOutcomes = c(-1, -.5, -.09);
safeOutcomes = c(0, .25, .5)

safePgam = 1/(1+exp(-1*( (typeCoef*0) + (amtCoef*safeOutcomes) + (intxnCoef*0*safeOutcomes) ))); 
gainPgam = 1/(1+exp(-1*( (typeCoef*1) + (amtCoef*gainOutcomes) + (intxnCoef*1*gainOutcomes) ))); 
lossPgam = 1/(1+exp(-1*( (typeCoef*-1) + (amtCoef*lossOutcomes) + (intxnCoef*-1*lossOutcomes) ))); 

plot(c(safePgam), type="l", col="green", lwd=3, ylim=c(0,1), main="Risk-taking changes as a function of past outcome type and amount", ylab="p(gamble)", xlab="past outcome", axes=F)
...
LEFT OFF HERE!! 6/14 - CREATED THE RECEIVED-NOT, UPDATED THE MODELS AND NOW NEED TO CHANGE THE PLOT
...
# PSH IDEA ABOUT WHAT IS HAPPENING WITH TRIAL AND EARNINGS: this figure below is technically showing the difference between earnings and trial because they have opposite effects 
...
levelTracking = vector()
for (s in 1:nSub) {
  sub = mriBehClean[mriBehClean$subjectIndex==subIDs[s],];
  tmpvec = cumsum(sub$groundEV);
