...
# RTs have a tail where there are a few people with long RTs that could be giving us this relationship. Let's sqrt it and then run the regression
mriBehClean$RTssqrt = sqrt(mriBehClean$RTs)

model_relEarnWeightedRTsqrtlmer = lmer(relativeEarningsWeighted~ RTssqrt+ (1|subjectIndex), data = mriBehClean);
summary(model_relEarnWeightedRTsqrtlmer); 

# model below switched outcome variable
model_relEarnWeightedRTsqrtlmer = lmer(RTssqrt~relativeEarningsWeighted+ (1|subjectIndex), data = mriBehClean);
summary(model_relEarnWeightedRTsqrtlmer); 
# still a positive relationship between RTs

#this is something we might need to put in the supplement or in the paper somewhere

# We aren't equipped to analyze RT here and figure out what exactly is going on. It looks like when earnings are more than expected, there is less variability in RTs (distribution is smaller) relative to when earnings are less than expected there is more variability. 
# .034 * .015 = .0005 (fast RT)
# .34 * 1.4 = .48 (slowest RT)

# worst rel. earnings = -.37 * .2 = -.07
# best rel. earnings = .30 *.2 = .06 

# we can look at deviations from the median RT given best and worst relative earnings
# .61-.07 = .54^2 = .29 (290ms)
# .61 + .06 = .67^2 = .45 (450ms)
# if RT is at median (.61^2 = 370ms) and if you're in the worst place you can be (relative earnings) then your RT will be a little faster (290ms) and if you're relative earnings are the best they could, RT will be a little slower (450ms)

# is this something that we should account for hemodynamically? We do have temporal derivatives and the difference here is very small (160ms difference TOTAL from best to worst earnings or 1/6th of a second). This suggests that we dont' need to be worried that this is messing up our BOLD model fit?

# it does mean that thinking about looking at relative earnings, there is potentially something going on with RTs. RCS is a good place to follow up (no forced viewing period) and our task here (forced viewing window) designs against potential relationship between earnings and RTs.
...
# there seems to be a very small trend that values rise toward this end. This could be a result of randomization and the run lengths (e.g. long and short runs are more likely to be in certain places of the task). This could be a thing to put in the supplement.

# calculate effect size for risky gain ~ trial:
# .024*.005 = .00012 # first trial
# .024*1 = .024 # last trial - there is a 2.4% shift in value (risky gain) over the course of the study

#Max gain is $61. So $61 * 2.4% = $1.46 change over the entire task (219 trials). Behaviorally, this doesn't really present an issue but for imaging, we do have to consider how this could impact us as we looking for signals of value (and here signals of value are correlated with time on a small level)

# calculate effect size for safe ~ trial:
# .008*.005 = .00004 # first trial
# .008*1 = .008 # last trial - less than 1% change in value (safe) over the course of the study or a chance of $0.49 in safe value.
...
predictedValues = predict(model1_trialLevel,type="response"); # this gets predicted responses (p(gamble)) on each trial from model one
mriBehClean$model1PredictedVals = predictedValues
...
pocNAind = which(is.na(mriBehClean$poc1scaled)); # rows where there is NA (i.e. where we are missing data from in the past outcome model)

# Let's add the past outcome predicted values to the mriBehClean data set (so we can see stuff easily by participant to get a better look at what's happening). This is a little tricky because the poc predicted values vector is shorter (because NAs are not included in the model). Below is a nasty loop to do that (I ~knowwww~ this is not sophisticated whatever bye!)
tmpvector = vector(mode="numeric", length=nrow(mriBehClean))
tmpvector[pocNAind] = NA

pocStart = 1 # initialize
for (t in 1:length(pocNAind)) {
  if(t < length(pocNAind)){
    start = pocNAind[t]+1
    end = pocNAind[t+1]-1
    len = end-start # number of trials between the NAs
    pocEnd = pocStart + len
    
    tmpvector[start:end] = predictedValuesOC[pocStart:pocEnd]
  
    pocStart = pocEnd+1
  } else {
    tmpvector[(end+2):length(tmpvector)] = predictedValuesOC[pocStart:length(predictedValuesOC)]
  }
}


mriBehClean$pocPredictedValues = tmpvector;

mriBehClean$triLev_poc_modelDiff_predictedVals = mriBehClean$model1PredictedVals - mriBehClean$pocPredictedValues
...
pdf("../figures/triLevModel_pocModel_predPgam_all.pdf") 
par(mfrow=c(1,1), pty="s")
plot(predictedValuesMod1, ylab="p(gamble)", xlab="trial", main="predicted risk-taking\ntrial-level model", pch=16, col=rgb(red=1, blue=0, green=0, alpha=.5))
abline(h=.5)

plot(predictedValuesOC, ylab="p(gamble)", xlab="trial", main="predicted risk-taking\npast outcome model", pch=16, col=rgb(red=0, blue=1, green=0, alpha=.5))
abline(h=.5)

plot(predictionDiff,ylab="diff in p(gamble)", xlab="trial", main="Model differences\ntrial-level - past outcome", pch=16, col=rgb(red=0, blue=0, green=1, alpha=.5))
abline(h=0)


for (s in 1:nSub) {
  par(mfrow = c(1,3), pty="s")
  plot(mriBehClean$model1PredictedVals[mriBehClean$subjectIndex==subIDs[s]], ylab="p(gamble)", xlab="trial", main="trial-level model", pch=16, col=rgb(red=1, blue=0, green=0, alpha=.5))
  abline(h=.5)
  
  plot(mriBehClean$pocPredictedValues[mriBehClean$subjectIndex==subIDs[s]], ylab="p(gamble)", xlab="trial", main="past outcome model", pch=16, col=rgb(red=0, blue=1, green=0, alpha=.5))
  abline(h=.5)
  
  plot(mriBehClean$triLev_poc_modelDiff_predictedVals[mriBehClean$subjectIndex==subIDs[s]], ylab="diff in p(gamble)", xlab="trial", main="trial-poc predicted", pch=16, col=rgb(red=0, blue=0, green=1, alpha=.5))
  abline(h=.0)
  
  mtext(sprintf("Predicted risk-taking: trial level vs. past outcome models, sub %.0f",subIDs[s]), side = 3, line = -2, outer = TRUE) 
}
