...
- choiceDispMeanEV_run#.txt: choice display modulated by mean expected value (mean center)
- choiceDispEVGAmt_run#.txt: choice display modulated by ev gain amount (mean center)
- choiceDispSafeAmt_run#.txt: choice display modulated by ev safe amount (mean center)
- choiceDispChosenUnchosen_run#.txt: choice display modulated by chosen-unchosen
...
- choiceDispPastOCAmt_run#.txt: choice display modulated by past outcome amount
- choiceDispEarnExpAmt_run#.txt: choice display modulated by earnings - expectations (using beta weights from behavior)
- choiceDispLinear_run#.txt: choice display modulated by a linear term (0 to 1 over the task)
...
RDMraw$relativeEarnings = (earningsBeta*RDMraw$earningsSC) + (expectationsBeta*RDMraw$expectations)
...
#for (s in 1) {  
  ...
  #for (r in 1){
    ...
    modulationEV = RDMsubRun$meanEV- mean(RDMsubRun$meanEV); # mean EV
    modulationEVG = (RDMsubRun$riskyGain*.5)-mean(RDMsubRun$riskyGain*.5); # ev gamble
    modulationSafe = RDMsubRun$alternative-mean(RDMsubRun$alternative); # safe amount
    modulationRelEarnings = RDMsubRun$relativeEarnings - mean(RDMsubRun$relativeEarnings) # relative earnings
    ...
    # if(length(missedTrials>0)){
    #   modulationChoseUnchosen = modulationChoseUnchosen[-c(missedTrials)] # remove missed trials
    # }
   ...
   # round and scale modulators
    ...
    choiceShiftNoModulation = cbind(onsetShift, durationShift, 1)
...
# for the EV that includes all trial types, we need to remove missed trials (missed Ts are already dealt with by the nature the gain/loss/safe outcome EVs)  
    ...
    # all outcomes
        modulationAllotc = RDMsubRun$outcome - mean(RDMsubRun$outcome); # outcome amount, mean centered
      ...
      # earnings 
        modulationEarningsTrialT = RDMsubRun$earningsTrialT - mean(RDMsubRun$earningsTrialT) # earnings including outcome on current trial
