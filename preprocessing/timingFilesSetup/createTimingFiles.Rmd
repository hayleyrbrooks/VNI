...
for (s in 1:nSubRaw) {
  ...
  for (r in 1:nRun){
    ...
    modulationEV = RDMsub$meanEV[RDMsub$block==r]- mean(RDMsub$meanEV[RDMsub$block==r], na.rm=T); # mean EV
    modulationGain = RDMsub$riskyGain[RDMsub$block==r]-mean(RDMsub$riskyGain[RDMsub$block==r], na.rm=T); # risky gain amount
    modulationSafe = RDMsub$alternative[RDMsub$block==r]-mean(RDMsub$alternative[RDMsub$block==r], na.rm=T); # safe amount
  ...
  for (r in 1:nRun){
    ...
    # modulations are mean-centered for each functional run for each participant
      # all outcomes
        ...
        modulationAllotc = RDMsub$outcome[RDMsub$block==r]-mean(RDMsub$outcome[RDMsub$block==r], na.rm=T); # outcome amount
  ...
  for (r in 1:nRun){
    ...
    # mean center modulation within each functional run for each participant
      # all decision types
        ...
        modulationEV = RDMsub$meanEV[RDMsub$block==r]-mean(RDMsub$meanEV[RDMsub$block==r], na.rm=T); # mean EV
        modulationGain = RDMsub$riskyGain[RDMsub$block==r]- mean(RDMsub$riskyGain[RDMsub$block==r], na.rm=T); # risky gain amount
        modulationSafe = RDMsub$alternative[RDMsub$block==r] - mean(RDMsub$alternative[RDMsub$block==r], na.rm=T); # risky safe amount
...
--> depending on what GLM we are running, these missed trial files may be slightly different because we are going to use 3-column format that specifies the timing
--> I think each missed trial document needs to be the same size the EV files for each GLM analysis.
--> Each row either has 1s (don't include this trial - its 'missing') or 0s (keep this trial)

These missed trial files are only necessary to complement onset files that use ALL trials (e.g. valuation period onset files, outcome onset file that uses all outcomes and decision period (all types of decision)). For the other variations - we don't need to complement with dummy EVs because none of the missed trials are included in those onset files because we subset those files based on participants' choice (i.e. if a participant missed a choice, there would be an NA for that choice and outcome and so they wouldn't have been pulled into those specific onset files above).
...
```{r}
# right now, these missed trial files are 73 rows long (for all trials).
# we will need to make these so that they match the analyses we want to do (where some trials are not included)
nTblock = 73;
nRun = 3;
offset = .6087;
...
for (s in 1:nSubRaw) {  
  ...
  for (r in 1:nRun){
    ...
    outcomeInd = which(eventrun$eventType=="outcome");# index start of outcome display for all outcomes
     ...
     # get the onset and duration for all outcomes
    onsetAllotc = eventrun$onset[outcomeInd]; # onset is outcome onset
    durationAllotc = eventrun$duration[outcomeInd]; # duration is outcome duration
    
    onsetAllotc = onsetAllotc + offset; # offset duration by .6087s


    # note the missed trials - first set everything to 0 
     missedTrialsRun = matrix(data=0, nrow = nTblock, ncol=1); # all zeroes
    ...
    # missedTrialsRun3 = matrix(data=0, nrow = nTblock, ncol=1); # all zeroes
    
    # pull out each run  
     run1 = RDMsub[RDMsub$block==r,];
    ...
    # index the missed trials
     missTrunInd = which(is.na(run1$choice)); 
    ...
    missedTrialsRun[missTrunInd] = 1
    #missedTrialsRun1[missTrun1ind] =1
    #missedTrialsRun2[missTrun2ind] =1
    #missedTrialsRun3[missTrun3ind] =1
    
    # make the 3 column matrices
    run1_3col = cbind(onsetAllotc, durationAllotc, missedTrialsRun)
    
    #copy the vectors above 2x to make 3 columns
    # run1_3col = cbind(missedTrialsRun1, 0, 0);
    # run2_3col = cbind(missedTrialsRun2, 0, 0);
    # run3_3col = cbind(missedTrialsRun3, 0, 0);
    
    # create files names and save vectors as txt files
    missTrunfileName = sprintf("%s/sub-%s/missedTrials_run%s.txt", config$path$data$missedTrials_files,subID_bids[s],r);
    ...
    write.table(run1_3col, file=missTrunfileName, row.names = F, col.names = F, sep="\t");
    # write.table(run1_3col, file=missTrun1fileName, row.names = F, col.names = F, sep="\t");
    # write.table(run2_3col, file=missTrun2fileName, row.names = F, col.names = F, sep="\t");
    # write.table(run3_3col, file=missTrun3fileName, row.names = F, col.names = F, sep="\t");
