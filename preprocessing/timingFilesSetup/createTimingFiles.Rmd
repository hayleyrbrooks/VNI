...
# CREATING ONSET FILES FOR PAST OUTCOME MEANS THAT ALL PARTICIPANTS HAVE TRIALS THAT NEED TO BE EXCLUDED IN RUN 1 BECAUSE EVERYONE WILL HAVE ATLEAST THE FIRST TRIAL POC =NAN. FOLLOWING OUR BEHAVIORAL ANALYSIS, WE AREN'T PUTTING NAN FOR FIRST OUTCOME ON EACH RUN, JUST RUN1 AT THE BEGINNING OF THE TASK. THIS MEANS WE HAVE TO MAKE THEIR MISSED TRIALS TIMING FILES SLIGHTLY DIFFERENT THAN WE DO IN SECTION BELOW. 
...
for (s in 1:nSubRaw) {
  ...
  for (r in 1:nRun){
    ...
    modulationPOC = RDMsub$poc1[RDMsub$block==r] - mean(RDMsub$poc1[is.finite(RDMsub$poc1) & RDMsub$block==r],na.rm=T); #poc mean centered
    modulationPOC = round(modulationPOC, digits=2)
    ...
    # for the EV that includes all trial types, we need to remove missed trials (missed Ts are already dealt with by the nature of subsetting the gamble/safe choices)
    missedTrialsPOConlyInd = RDMsub$triBlock[RDMsub$block==r & is.na(RDMsub$poc1)]# | is.na(RDMsub$choice)]; # index the missed trials for this run
    if (length(missedTrialsPOConlyInd)>1){
      missedTrials = c(missedTrialsPOConlyInd, missedTrialsPOConlyInd[2:length(missedTrialsPOConlyInd)]-1); # get indices of of where poc and choice =NaN
      # only do this for participants who missed choices (all participants will at least have the first trial poc =NaN)    
    } else {
      missedTrials = missedTrialsPOConlyInd;
    }; 
   ...
   #print(missedTrials)
    ...
    decisionPOCnoMod = cbind(onsetAllDecision, durationAllDecision,1); # one for parametric modulation to account for main effects of past outcome (not sure if we will use this in the analysis - making just in case)
