...
# earnings are 0 to 1 across the entire task (not within runs or starting over at the beginning of each run)
# expectations are scaled across individuals (i.e. by the single largest cumulative earnings)
# earnings scaled by max earnings across participants beta =  2.3959 
# expectations beta = -2.1927 
# from model: model5_potc_posshift_earn_trial 
# LEFT OFF MAKING EARNINGS - EXPECTATIONS - models in vni beh previously used earnings normalized by each individual but should we do that or use earnings scaled? After sorting this out, continue below making the earnings-expectations timing files.
...
for (s in 1:nSub) {
  ...
  earningsSub = cumsum(tmpOutcomeVec);
  expScaled = sub$trial/max(sub$trial)
  
  earningsSubScaled =cumsum(tmpOutcomeVec)/max(cumsum(tmpOutcomeVec)) # this normalizes earnings within each participant
  
  earnings = c(earnings,earningsSub);
  expectations = c(expectations,expScaled);
  earningsScaledBySub = c(earningsScaledBySub,earningsSubScaled)
  
}

RDMraw$expectations = expectations # store linear expectation
...
#for (s in 1) {  
  ...
  #for (r in 1){
    ...
    #remove missed trials (for these timing files we are only accounting for choice display when choice was made - missed trials are accounted for down below)
   ...
   # round and scale modulators
    ...
    # the chosen v unchosen onset and duration is a little different than above because we are accounting for choice
    ...
    choiceShiftAmtNoModfileName = sprintf("%s/sub-%s/%s.txt", config$path$data$onset_files,subID_bids[s],paste("choiceDispShiftAmtNoMod_run",r,sep=""))
    ...
    write.table(choiceShiftAmt, file=choiceShiftAmtNoModfileName, row.names = F, col.names = F, sep="\t");
