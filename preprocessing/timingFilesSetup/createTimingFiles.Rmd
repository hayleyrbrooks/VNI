...
#for (s in 1) {  
  ...
  #for (r in 1){
    ...
    # add to RDM data
    RDMsubRun$onset = choiceDispOnset
    RDMsubRun$duration = choiceDispDuration
   ...
   # round and scale modulators
    ...
    #LEFT OFF HERE! I COMBINED THE RDM WITH ONSET/DURATION - NOW THIS SHOULD MAKE SUBSETTING MORE STRAIGHTFORWARD
    ...
    RDMsubClean = RDMsub[is.finite(RDMsub$choice),] # remove the missed trial (this will make sure the indices are same for our onset/duration variables)
    shiftTrials = RDMsubClean$triBlock[RDMsubClean$positiveShiftSC>0 & RDMsubClean$block==r] # get trial were positive shift occurred
    onsetShift = onset[shiftTrials] # subset onset when shift occurred
    durationShift = duration[shiftTrials] # subset duration when shift occurred
    meanShiftAmt = mean(RDMsub$positiveShiftSC[is.finite(RDMsub$choice) & RDMsub$positiveShiftSC>0 & RDMsub$block==r])  # calculate mean positive shift amount
    modulationShift = RDMsub$positiveShiftSC[is.finite(RDMsub$choice) & RDMsub$positiveShiftSC>0 & RDMsub$block==r] - meanShiftAmt # mean center the modulator
    modulationShift = round(modulationShift, digits=2) # round to 2 decimals (this variable is already scaled)
    
    pocTrials = RDMsub$triBlock[is.finite(RDMsub$choice) & is.finite(RDMsub$poc1) & RDMsub$block==r] # get trials where there was a past outcome
    onsetPOC = onset[pocTrials] # subset onset where there was a past outcome
    durationPOC = onset[pocTrials] # subset duration where there was a past outcome
    meanPOCAmt = mean(RDMsub$poc1[is.finite(RDMsub$choice) & is.finite(RDMsub$poc1) & RDMsub$block==r]) # calculate mean poc
    modulationPOC = RDMsub$poc[is.finite(RDMsub$choice) & is.finite(RDMsub$poc1) & RDMsub$block==r] - meanPOCAmt # mean center the modulator
    ...
    choiceEVG = cbind(onset, duration, modulationEVG);
    ...
    choiceChosenUnchosen = cbind(onset, duration, modulationChoseUnchosen)
