...
for (s in 1:nSubRaw) {
  ...
  for (r in 1:nRun){
    ...
    chosenInd = which(is.finite(RDMsub$choiceRecode)  & RDMsub$choiceRecode==1 & RDMsub$block==r) # this pulls out ind in 1-3 rounds (does not start at ind = 1 as rounds increase)
    unchosenInd = which(is.finite(RDMsub$choiceRecode)  & RDMsub$choiceRecode==-1 & RDMsub$block==r) # (does not start at ind = 1 as rounds increase)
    
    meanEVDiff = mean((RDMsub$riskyGain[is.finite(RDMsub$choiceRecode) & RDMsub$block==r]*.5) - RDMsub$alternative[is.finite(RDMsub$choiceRecode) & RDMsub$block==r])
    
    #LEFT OFF ABOVE WITH DOING ONE TIMING FILE = 1*(EVG-EVS) OR -1*(EVG-EVS) ^^^^^
   ...
   # round and scale modulators
    ...
    choiceChosen = cbind(onset, duration, modulationChosen)
    choiceUnchosen = cbind(onset, duration, modulationUnchosen)
    ...
    choiceChosenfileName = sprintf("%s/sub-%s/%s.txt", config$path$data$onset_files,subID_bids[s],paste("choiceDispChosen_run",r,sep=""))
    choiceUnchosenfileName = sprintf("%s/sub-%s/%s.txt", config$path$data$onset_files,subID_bids[s],paste("choiceDispUnchosen_run",r,sep=""))
