...
# create earnings and expectation variables (these are used in both the choice display and outcome display timing files)
...
RDMraw$earningsTrialT = earningsTrialT; # store earnings that include current trial outcome
RDMraw$earningsTrialTsc = RDMraw$earningsTrialT/max(RDMraw$earningsTrialT) # scale earnings (inc oc on trial t) and store it

# CHOICE DISPLAY: using earnings up until trial t for relative earnings at outcome display
...
# OUTCOME DISPLAY: using earnings including trial t for relative earnings at outcome display
RDMraw$relativeEarningsOCdisp = (earningsBeta*RDMraw$earningsTrialTsc) + (expectationsBeta*RDMraw$expectations); # using addition because expectations are negative


# now create the timing files for choice display
for (s in 1:nSubRaw) {
#for (s in 1) {  
...
- outcomeDispRelativeEarnings_run#.txt: outcome display modulated by earnings - expectations (using beta weights from behavior; mean-centered across runs)
- outcomeDispLinearTerm_run#.txt: outcome display modulated by a linear term (0 to 1 over the task; mean-centered across runs)
...
# for the EV that includes all trial types, we need to remove missed trials (missed Ts are already dealt with by the nature the gain/loss/safe outcome EVs)  
    ...
    # earnings 
    modulationEarningsTrialT = RDMsubRun$earningsTrialTsc - mean(RDMsub$earningsTrialTsc) # earnings including outcome on current trial mean centered ACROSS runs
    modulationRelEarnTrialT = RDMsubRun$relativeEarningsOCdisp - mean(RDMsub$relativeEarningsOCdisp) # relative earnings including outcome on current trial mean centered ACROSS runs
    modulationLinear = RDMsubRun$expectations - mean(RDMsub$expectations) # linear term (0ish to 1 across the task) mean centered ACROSS runs
    ...
    outcomeDispRelativeEarnings = cbind(onsetAllotc, RDMsubRun$outcomeDispDuration, modulationRelEarnTrialT);
    outcomeDispLinearTerm = cbind(onsetAllotc,RDMsubRun$outcomeDispDuration, modulationLinear)
    ...
    outcomeDispRelativeEarningsfileName =sprintf("%s/sub-%s/%s.txt", config$path$data$onset_files,subID_bids[s],paste("outcomeDispRelativeEarnings_run",r,sep=""));
    outcomeDispLinearTermfileName =sprintf("%s/sub-%s/%s.txt", config$path$data$onset_files,subID_bids[s],paste("outcomeDispLinearTerm_run",r,sep=""));
    ...
    write.table(outcomeDispRelativeEarnings, file = outcomeDispRelativeEarningsfileName, row.names = F, col.names = F, sep="\t");
    write.table(outcomeDispLinearTerm, file = outcomeDispLinearTermfileName, row.names = F, col.names = F, sep="\t");
