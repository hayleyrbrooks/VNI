...
- parametric modulations are mean-centered by participant across all runs?
...
for (s in 1:nSubRaw) {
  ...
  for (r in 1:nRun){
    ...
    # store events by onset, duration and modulation (mean ev, gain, safe) and mean center them
    ...
    modulationEV = RDMsub$meanEV[RDMsub$block==r]-mean(RDMsub$meanEV, na.rm=T); # mean EV
    modulationGain = RDMsub$riskyGain[RDMsub$block==r]-mean(RDMsub$riskyGain, na.rm=T); # risky gain amount
    modulationSafe = RDMsub$alternative[RDMsub$block==r]-mean(RDMsub$alternative, na.rm=T); # risky safe amount
    
  # ^right now, this mean-centers across all runs.
