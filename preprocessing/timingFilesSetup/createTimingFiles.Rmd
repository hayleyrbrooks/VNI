...
# These timing files are a little different from others in that we have to account for past outcomes that = NaN because the participant missed the trial. The way we are doing this is to set those past outcomes = 0 (versus doing something like making them missed trials)
...
for (s in 1:nSubRaw) {
  ...
  for (r in 1:nRun){
    ...
    missedTrials = RDMsub$triBlock[is.na(RDMsub$choice) & RDMsub$block==r] # index missed trials for this run
     if(length(missedTrials>0)){
      onsetAllDecision = onsetAllDecision[-c(missedTrials)]; # remove missed trials
